describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const productItem = await page.$$('product-item');
    for (let i = 1; i < productItem.length; i++) {
      const shadowRoot = await productItem[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      await button.click();
    }
    ...
    expect(cartCountValue['_remoteObject'].value).toBe('20');
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const localStorage = await page.evaluate(() => { return localStorage.getItem('cart'); });
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartCount = await page.$('#cart-count');
    const cartCountValue = await cartCount.getProperty('innerText');
    expect(cartCountValue['_remoteObject'].value).toBe('0');
  }, 10000);
